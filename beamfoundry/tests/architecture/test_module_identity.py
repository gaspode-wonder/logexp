...
def test_no_duplicate_module_identities():
    ...
    diagnostics_packages = [name for name in sys.modules if name.endswith("bp.diagnostics")]
    ...
    assert len(diagnostics_packages) == 1, (
        f"Diagnostics package imported under multiple identities: " f"{diagnostics_packages}"
    )
