# Makefile — Beamfoundry (canonical, deterministic)
...
dev-fast:
	$(call timed,"Starting Flask (fast mode)", \
		$(ACTIVATE) && \
		env FLASK_APP=app:create_app flask run --reload \
	)

shell:
	$(call timed,"Opening Flask shell", \
		$(ACTIVATE) && env FLASK_APP=app:create_app flask shell \
	)
...
db-reset:
	$(call timed,"Resetting development DB", \
		rm -f app/data/readings.db && \
		$(ACTIVATE) && env FLASK_APP=app:create_app flask db upgrade \
	)
...
db-migrate:
	$(call timed,"Creating migration", \
		$(ACTIVATE) && env FLASK_APP=app:create_app flask db migrate \
	)
...
test-clean:
	$(call timed,"Cleaning __pycache__", find app analytics parsers validation -type d -name "__pycache__" -exec rm -rf {} +)
...
doctor:
	$(call timed,"Doctor checks", \
		...
		(test -f app/data/readings.db && echo "DB exists" || echo "DB missing") \
	)
