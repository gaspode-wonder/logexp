# logexp/app/services/database_diagnostics.py
...
def get_database_status():
    ...
    Performs connection tests, schema checks, and row counts,
    then delegates to the pure diagnostics function.
    """
    ...
    try:
        readings_count = db.session.query(Reading).count()
    ...
    # Schema check
    try:
        db.session.execute(text("SELECT * FROM {Reading.__tablename__} LIMIT 1"))
        schema_ok = True
    except Exception:
        schema_ok = False
