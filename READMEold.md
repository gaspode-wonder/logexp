...
This project uses **PostgreSQL + SQLAlchemy + Flask‑Migrate (Alembic)**.  
If you ever upgrade Postgres, switch clusters, or need a clean schema rebuild, follow this guide to safely reset your environment.

---

## ✅ 1. Verify Flask Is Using the Correct `DATABASE_URL`

The application reads your database connection string from the `DATABASE_URL` environment variable.
...
```sql
postgresql://loginname@localhost:5432/Experiments
```
...
If this prints the correct URL, Flask is wired to the right database.

If it prints `None`, export the variable in your shell:
...
If you want a fresh database (recommended after upgrading Postgres):
...
```sql
Experiments | loginname | UTF8 | ...
```

At this point you have:

- A clean Postgres 18 database  
- A valid role (`loginname`)  
- A correct connection string  
- No leftover schema from older clusters  

---

## ✅ 3. Rebuild Migrations and Schema From Scratch

If your migrations were created under an older Postgres version or the schema is out of sync, reset them.
...
### **Generate a fresh migration from your models**
```bash
flask db migrate -m "initial schema for Experiments"
```

This scans your SQLAlchemy models and generates a new migration file.
...
```sql
 logexp_readings | table | loginname
```

(or whatever tables your models define)
...
- A clean Postgres 18 database  
- A correct `DATABASE_URL`  
- A fresh migration history  
- A schema that matches your SQLAlchemy models  
- A Flask app that connects cleanly and reliably  

This workflow is the recommended way to reset your environment whenever you upgrade Postgres or need a reproducible schema rebuild.
